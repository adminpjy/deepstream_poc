# file:/workspace/webmain.py (with Integrated Logging)
import cv2
import time
import numpy as np
import config
...
app = Flask(__name__, template_folder='.') # template_folder='.' 表示在當前目錄找模板
...
def log_stream():
    def generate_logs():
        while True:
            if log_queue:
                log_entry = log_queue.popleft().replace("\n", "<br>")
